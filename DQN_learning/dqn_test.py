...
class Net(nn.Module):
    def __init__(self):  # 定义Net的一系列属性
        ...
        self.fc1 = nn.Linear(N_STATES, 30)  # 设置第一个全连接层(输入层到隐藏层): 状态数个神经元到30个神经元
        self.fc1.weight.data.normal_(0, 0.1)  # 权重初始化 (均值为0，方差为0.1的正态分布)
        self.out = nn.Linear(30, N_ACTIONS)  # 设置第二个全连接层(隐藏层到输出层): 30个神经元到动作数个神经元
...
class DQN(object):
    ...
    def choose_action(self, x):  # 定义动作选择函数 (x为状态)
        ...
        if np.random.uniform() < EPSILON:  # 生成一个在[0, 1)内的随机数，如果小于EPSILON，选择最优动作
            ...
            np.random.shuffle(action) # shuffle them
